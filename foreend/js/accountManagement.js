...
function dataEntry(box, data){
	for (var i = 0; i < data.length; i++) {
		box.append(createTr(data[i]['questionnaire_title'], data[i]['questionnaire_id']),data[i]['user_name']);
	}
}
// 创建一行表格
function createTr(title, id, username){
	console.log(username);
	 let tr = `<tr>
				  <td><a href="#" onclick='modelDetail(this)'> ${username}</a></td>
